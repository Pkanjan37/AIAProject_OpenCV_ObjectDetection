...
void Aia2::getContourLine(const Mat& img, vector<Mat>& objList, int thresh, int k){
	...
	cout << "The threshold = " << thresh << endl;
	cout << "Now binarizing...";
		
	Mat binImg = Mat::zeros(img.rows, img.cols, CV_32FC1);

	// binarize image (depending on threshold thresh)
	threshold(img, binImg, thresh, 255, THRESH_BINARY);

	// erode the binarized image
	Mat erodImg = Mat::zeros(img.rows, img.cols, CV_32F);
	if (k > 0) {
		erode(binImg, erodImg, getStructuringElement(MORPH_RECT, Size(k, k)));
	}
	else
		cout << "iteration k for erosion is not set" << endl;

	// get all the contours, put each vector of a contour (2-channel) in objList
	findContours(binImg, objList, CV_RETR_LIST, CV_CHAIN_APPROX_NONE);
...
void Aia2::plotFD(const Mat& fd, string win, double dur){
	...
	//dft(...)
	//? Note : fd might be rotation - normalized and thus real - valued
	//? transform to 2 - channel matrix with zero imag.part
		
	//- Create image of corresponding size
	//Note : fd might be transl. - normalized ? neg.coordinates
	//Note : fd might be scale - normalized ? scale to standard size
	//Mat::zeros(...)
	
	//Set corresponding pixel values to white
	
	//Create window and display image : cv::imshow(…)
